...
2. Word onset timestamps match the audio features
3. Transcription segments align with acoustic events

Creates a multi-panel plot showing:
- Panel 1: Audio envelope with word onset markers
- Panel 2: F0 contour with word onset markers
...
def load_word_onsets(subject, run, base_dir):
    ...
    # Extract all word onsets with their MEG-aligned times
    word_onsets = []
    for _, seg in transcript.iterrows():
        ...
        for w in words_data:
            ...
            # CORRECTED: meg_time = audio_time + offset
            # The offset is already baked into seg_start_meg
...
def verify_sync_alignment(subject, run, base_dir, time_window=None, zoom_window=None):
    """
    Create diagnostic visualization for sync alignment verification.
    ...
    """
    print(f"\n{'='*70}")
    print(f"VERIFYING SYNC ALIGNMENT: {subject} run-{run:02d}")
    print(f"{'='*70}\n")
    ...
    fig.suptitle(f'Sync Alignment Verification: {subject} run-{run:02d}\n' +
                 f'Sync offset = {sync_offset:.4f}s ({"+" if sync_offset >= 0 else ""}{sync_offset:.3f}s)',
    ...
    ax.set_title('Audio Envelope + Word Onsets (red lines)', fontsize=10)
    ...
    ax.grid(True, alpha=0.3)
    ax.legend(loc='upper right')
    ax.set_title('F0 Contour + Word Onsets (red lines)', fontsize=10)
    ...
    ax.grid(True, alpha=0.3)
    ax.legend(loc='upper right')
    ax.set_title(f'Zoomed View: {z_start:.2f}s - {z_end:.2f}s (MEG time)', fontsize=10)
    ...
    output_file = output_dir / "sync_verification.png"
    ...
    print(f"   Word onset range (MEG time): {words_df['time_meg'].min():.2f}s to {words_df['time_meg'].max():.2f}s")

    # Check 2: Word onset alignment
    print(f"\n2. Word Onset Alignment:")
...
def main():
    parser = argparse.ArgumentParser(description='Verify audio-MEG synchronization alignment')
